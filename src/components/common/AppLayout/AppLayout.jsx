  ...
  return (
    <div className="min-h-dvh bg-slate-50">
      ...
      <aside
        ...
        className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-200 bg-white p-5 transition-transform motion-reduce:transition-none lg:translate-x-0 ${open ? "translate-x-0" : "invisible -translate-x-full lg:visible"}`}
      >
        <div className="mb-9 flex items-center gap-3 px-2 py-2">
          <span className="flex size-10 items-center justify-center rounded-xl bg-emerald-700 text-lg font-bold text-white">
            A
          </span>
          <div>
            <p className="text-lg font-bold tracking-tight">AMB Sales</p>
            <p className="text-xs text-slate-500">Your everyday business</p>
          </div>
          <button
        ...
        <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto">
          {links.map(({ path, name, Icon }) => (
            <NavLink
              ...
              onClick={() => setOpen(false)}
              className={({ isActive }) =>
                `flex min-h-11 items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-colors ${isActive ? "bg-emerald-50 text-emerald-800" : "text-slate-600 hover:bg-slate-50 hover:text-slate-950"}`
              }
            >
        ...
        <div className="mt-5 border-t border-slate-100 pt-5">
          ...
          <button
            onClick={logout}
            className="mt-3 flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm text-slate-600 hover:bg-red-50 hover:text-red-700"
          >
            <FaSignOutAlt />
            Sign out
          </button>
      ...
      <div className="min-w-0 lg:pl-72" inert={open ? true : undefined}>
        <header className="sticky top-0 z-20 flex h-20 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur-sm sm:px-8">
          <button
            ref={trigger}
          ...
          <div
            className="ml-auto flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-800"
